            ...
            try
            {
                if (_modelCache.TryGetValue(fileName, out cacheEntry))
                {
                    if (cacheEntry.Model != null)
                    {
                        ...
                        Instantiate(cacheEntry.Model, external3DModel.ModelParentTransform)
                            .SetActive(true);
                ...
                Instantiate(cacheEntry.Model, external3DModel.ModelParentTransform).SetActive(true);
                external3DModel.OnLoadCompleted();

                foreach (External3DModel otherModel in cacheEntry.ModelLoadQueue)
                {
                    Instantiate(cacheEntry.Model, otherModel.ModelParentTransform).SetActive(true);
